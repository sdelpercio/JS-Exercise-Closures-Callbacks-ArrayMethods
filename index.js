...
function processSum(numberList, callback) {
  return callback(numberList.reduce((accumulator, item) => {
...
function getFullNames(runners) {
  ...
  runners.forEach((item) => {
    return runnerNames.push(`${item.last_name}, ${item.first_name}`);
  });
  return runnerNames;
}
...
function tallyUpDonations(runners) {
  return runners.reduce((accum, current) => accum + current.donation, 0);
}
